...
class CodexExecAdapterTests(unittest.TestCase):
    ...
    def test_codex_exec_adapter_handles_large_single_json_line(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            fake = tmp / "fake_codex"
            fake.write_text(
                textwrap.dedent(
                    ...
                    print(json.dumps({{"type": "assistant_delta", "delta": "x" * 70000}}), flush=True)
                    if last is not None:
                        last.write_text("large line final", encoding="utf-8")
                    """
                ),
                encoding="utf-8",
            )
            ...
            self.assertEqual(result.final_text, "large line final")
            self.assertTrue(any(event.kind == EventKind.ASSISTANT_DELTA for event in result.events))
